...
#warning remove this
    ...
    if (advertisedDevice.getAddress() == BLEAddress("c0:4e:30:85:53:4e")) // Testing purposes
    {
      // if (!player.isActive())
      // {
      ...
      { // Check if the distance is within 20 cm
        ...
        beaconFound = true;
        beaconFound2 = false;
        // Serial.printf("Value of beaconFound = %d", beaconFound);
        // Serial.printf("Value of beaconFound2 = %d", beaconFound2);
        // audioPlaybackComplete = false;
      }
    }
    else if (advertisedDevice.getAddress() == BLEAddress("10:91:a8:26:12:72"))
    {
      if (distance <= 0.2)
      { // Check if the distance is within 20 cm
        // player.begin();
  ...
  while (1)
  {
    ...
    /*
      if (audioPlaybackComplete)
      {
        ...
        audioPlaybackComplete = false;
        Serial.println("AudioCompleteFalse");
...
void loop()
{
  // Serial.printf("Value of beaconFound = %d \r\n", beaconFound);
  // Serial.printf("Value of beaconFound2 = %d\r\n", beaconFound2);
  ...
  if (beaconFound2 == true && current != PLAYING_B2)
  {
    ...
    // while (beaconFound == false)
    //{
    //   if (player.copy() == 0)
    //   {
    //     Serial.println("Exiting Audio 2");
    //     break;
    //   }
    // } // Play the audio
    beaconFound2 = false;
  ...
  else if (beaconFound == true && current != PLAYING_B1)
  {
    ...
    // while (beaconFound2 == false)
    //{
    //   if (player.copy() == 0)
    //   {
    //     Serial.println("Exiting Audio 1");
    //     break;
    //   }
    // } // Play the audio
    // audioPlaybackComplete = true;
